...
import { User, Phone, Lock, Eye, EyeOff, KeyRound } from 'lucide-react';
import { motion } from 'framer-motion';

export default function FarmerFirstLogin() {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const response = await axios.post(`${baseUrl}/api/v1/auth/farmer/first-login`, {
      ...
      if (response && response.data && response.data.success) {
        showToast('Password created successfully! Please log in with your new credentials.', 'success');
        navigate('/farmer/login');
      }
    } catch (error) {
      const serverMessage = error.response?.data?.message || 'Activation failed. Please check your credentials.';
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-900 px-4 transition-colors duration-300">
      ...
      >
        ...
        <div className="text-center space-y-2 select-none">
          ...
          <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">First-Time Portal Setup</h2>
          <p className="text-xs text-slate-500">Enter your Farmer ID and Registered Mobile Number to activate your portal access and set your password.</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          {/* Password */}
          <div className="space-y-1.5">
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Create Password</label>
            <div className="relative">
          ...
          >
            {loading ? 'Activating...' : 'Activate Portal Account'}
          </button>
        </form>
